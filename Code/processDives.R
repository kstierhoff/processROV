...
source(here("Doc/settings", "settings_SanClemente_Abalone_2019.R"))

# Query starting IDs for all database tables ####
db.con <- odbcConnectAccess2007(db.dir)
nav.seed    <- as.numeric(sqlQuery(db.con, "SELECT max(dbo_tbl_NAV.nav_id) AS nav_seed
                                   FROM dbo_tbl_NAV;")[1] + 1)
event.seed  <- as.numeric(sqlQuery(db.con, "SELECT max(dbo_tbl_EVENTS.event_id) AS event_seed
                                   FROM dbo_tbl_EVENTS;")[1] + 1)
photo.seed  <- as.numeric(sqlQuery(db.con, "SELECT max(dbo_tbl_PHOTO_INFO.photo_id) AS photo_seed
                                   FROM dbo_tbl_PHOTO_INFO;")[1] + 1)
ctd.seed    <- as.numeric(sqlQuery(db.con, "SELECT max(dbo_tbl_CTD_CASTS.ctd_id) AS ctd_seed
...
# List all WinFrog project directories
# d <- sort(dir(dat.root, recursive = F, pattern = "\\d{2}-\\d{3}\\w", full.names = T))
d <- dir_ls(dat.root, recurse = FALSE, regexp = "\\d{2}-\\d{3}\\w")

# Subset directories to be processed
dir.list <- d[grep(dir.start, d):grep(dir.end,d)]

# create status bar
pb <- winProgressBar(title = "ROV File Processing Progress", 
...
for (i in dir.list) {
  ...
  invisible(file_copy(here("Code/processDives.R"), i, overwrite = TRUE))
  ...
  if (adjust.pitch) {
    ...
    write.csv(pr.data, 
              file = file.path(i, paste(dive.name,"PitchRollData.txt", sep = "_")),
              row.names = FALSE, quote = FALSE, na = "-999") 
  }
  
  # Process CTD data ----------------------------------------------------------
  if (ctd.on) {  
    # Process CTD data and calculate oxygen saturation
  ...
  if (length(which(is.na(DAT$altitude) == FALSE)) > 0) {
  ...
  DAT <- DAT %>% 
    mutate(
      camera_altitude = width.df$camera_altitude,
  ...
  write.csv(DAT.write, 
            file = file.path(i, paste(dive.name, "NavData.txt",sep = "_")), 
            quote = FALSE,row.names = FALSE, na = "-999")
  ...
  ctd.p 	<- ggplot(ctd.gg,aes(x = date_time, y = value, group = variable)) + 
    ...
    labs(title = paste0("CTD Data from transect ", dive.name, "\n")) + 
...
write.csv(DAT.temp,file = file.path(dat.root,"_Results", paste(dir.start,dir.start,"NavData.txt", sep = "_")),
          row.names = F, quote = F, na = "-999")

# Write PHOTO data for all dives to text file
if (nrow(PHOTO.temp) > 0) {
  write.csv(PHOTO.temp,file = file.path(dat.root,"_Results", paste(dir.start,dir.start,"PhotoInfo.txt", sep = "_")),
            row.names = F,quote = F, na = "-999")
}

# Write LOG data for all dives to text file
if (nrow(LOG.temp) > 0) {
  write.csv(LOG.temp,file = file.path(dat.root,"_Results", paste(dir.start,dir.start,"DiveEvents.txt", sep = "_")),
            row.names = F, quote  = F, na = "-999")
}
# Write CTD data for all dives to text file
if (nrow(CTD.temp) > 0) {
  write.csv(CTD.temp,file = file.path(dat.root,"_Results", paste(dir.start,dir.start,"CTDCasts.txt", sep = "_")),
            row.names = F,quote = F,na = "-999")
}
...
st_write(transects, file.path(dat.root,"_Results", paste(dir.start, dir.start, "Transects.shp", sep = "_")), 
         delete_layer = TRUE)

# Report new indices for next dive ####
cat(paste("Finished processing dives", dir.start, "to", dir.start, "\n", sep = " "),

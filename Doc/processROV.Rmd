...
dir_create(str_replace(dir.list, dat.root, here("Data/PHOTOS")))
  ...
  for (i in dat.dirs) {  
    ...
    if (log.info$size == 0) {
      ...
      if (nrow(DAT.tmp) > 0) {
        ...
        LOG.tmp$nav_id	<- NA_real_
        LOG.tmp$lag_s 	<- NA_real_
      }
    ...
    if (process.photos) {
      ...
      if (length(grep(dive.name, p)) > 0) { 
        ...
        if (nrow(DAT.tmp) > 0) {
          ...
          PHOTO.tmp$nav_id	<- NA_real_
          PHOTO.tmp$lag_s 	<- NA_real_
        }
...
dir_create(here("Output/_Results"), c("CameraData","PhysData","CTD_Casts","NavData","LatLongData"))
